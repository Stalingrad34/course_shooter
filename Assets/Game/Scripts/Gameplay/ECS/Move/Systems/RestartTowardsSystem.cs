...
{
  public class RestartTowardsSystem : IEcsRunSystem
  {
    private EcsFilter<IdentifierComponent, TransformComponent, MoveTowardsComponent> _filter;
      ...
      foreach (var i in _eventFilter)
      {
        ...
        foreach (var ii in _filter)
        {
          if (_filter.Get1(ii).Id == restartInfo.id)
          {
            ...
            _filter.Get3(ii).Destination = position;
          }
        }
      }
    }
